...
export default async function TempahBaharuPage() {
  const session = await auth();
  if (!session?.user?.id) redirect("/login");

  // Senarai fasiliti TIDAK ditapis ikut skop di sini (elak duplikasi logik
  // scopeMatches dalam query) — createBooking() semak can() sebenar semasa
  // submit, tolak dengan mesej jelas kalau di luar skop pemohon.
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-xl font-semibold">{ms.tempahan.tempahBaharu}</h1>
      <BookingForm
